...
void Manager (int which) {
    ...
    while(true)
    {
        //Stay active until there are no more people in need of elevator

        //Elevator is on ground floor
        if (eThread.currentFloor == 1)
        {
        ...
        //Elevator is on top floor
        else if (eThread.currentFloor == eThread.numFloors)
        {
...
void Person (int person) {
    ...
    while(true)
    {
        //Stay active until person arrives at desired floor
        e->Acquire();
        ...
        //Odd case where a persons current and desire floor are the same
        if (p->atFloor == p->toFloor && readyEnter)
        {
            q->P();
        ...
        //Person can go in the elevator
        else if (!in && readyEnter && direction && capacity)
        {
            q->P();
            if (goingUp)
            {
        ...
        //Person in elevator arrived to desired floor
        else if (in && readyLeave)
        {
            q->P();
	    if (goingUp)
...
void ArrivingGoingFromTo(int atFloor, int toFloor){
    ...
    Thread *person = new Thread("Creating Person Thread");
